...
### 2026-09-24
- 퀴즈 점검 및 고도화
  - CDN 전환 여파 보완: 다음 두 문제의 그림을 미리 받아 두고(prefetch), 모든 그림에 150ms 페이드 등장과 디스크 캐시 지정
  - 채점 피드백에 움직임 추가: 정답 보기는 통통 부풀고 오답 보기는 좌우로 떨림
  - 틀렸을 때는 1.5초(정답은 0.7초) 보여 줘서 정답을 확인할 시간을 확보
  - 진행바가 스르륵 차오르고, 연속 정답 배지는 개수가 늘 때마다 튀어오름
  - 듣기 문제 스피커 버튼에 은은한 고동(물결) 효과로 다시 듣기 유도
  - 결과 화면: 점수가 0에서 차오르는 카운트업 + 별 셋 등급 (90/60/30점 기준)
  - 보기 버튼 눌림 상태를 뉴모피즘 눌림 그림자(shadow-neu-pressed)로 통일
