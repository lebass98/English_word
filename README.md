...
- 저장 버튼은 북마크 아이콘 옆에 글자를 가로로 놓고 글자 크기를 양옆 판정 버튼과 같은 14px 로 맞췄다. 자리를 만들려고 세 버튼의 좌우 여백을 4px 로 같게 맞추고 사이 간격도 줄임
- 담기면 글자가 한 자 늘어나므로('저장'→'저장됨') 긴 쪽에 맞춰 저장 버튼 너비를 84px 로 잡고 shrink-0 을 줌. flex 행에서는 width 가 시작 크기일 뿐이라 shrink-0 이 없으면 자리가 모자랄 때 버튼이 줄어들어 '저장됨' 이 잘린다
...
- 학습 화면 상단 영어 단어 최대 크기를 24px → 31px 로 30% 키움
