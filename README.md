...
- 중3 59단원 빠진 그림 10장 선형그래픽으로 생성 및 등록 (머리·몸통 분리 개정 스킬 적용)
  - 대상: exclaim, driver, serve, galaxy, emigrate, fake, from time to time, ambitious, forehead, gradual
...
- 중3 52단원 빠진 그림 11장 선형그래픽으로 생성 및 등록 (머리·몸통 분리 개정 스킬 적용)
  - 대상: odd, pure, religion, move on, after all, take part in, grateful, attractive, envy, maintain, look into
...
- 중3 48단원 빠진 그림 2장 선형그래픽으로 생성 및 등록 (머리·몸통 분리 개정 스킬 적용)
  - 대상: dairy, be afraid of
...
- 메인에서 이모지 아이콘을 모두 지움 (연속 학습·오늘의 기록·그림 자리·그림 현황 배지). 언어 깃발은 남김
- 글꼴을 프로젝트 안에 설치해서 쓰도록 바꿈 (바깥 CDN 을 타지 않는다)
  - 일본어 화면은 Noto Sans JP, 한국어·영어 화면은 프리텐다드 GOV
  - 웹은 `public/fonts` 의 가변 woff2 를 @font-face 로, 네이티브는 `assets/fonts` 의 otf·ttf 를 expo-font 로 불러온다
  - React Native 는 글꼴이 상속되지 않아 Text 가 그려질 때 기본 글꼴을 깔아 준다 (`src/lib/appFont.ts`)
- 그림 현황판을 참고 시안대로 다시 만듦
  - 빈 자리(3,871칸)와 실제로 그려야 할 그림(2,715장)을 나눠서 보여준다. 같은 뜻을 가리키는 낱말은 그림 한 장을 함께 쓰기 때문
  - 코스마다 진행 단계(초기/진행 중/완성), 완성 유닛 수, 유닛 칸 격자를 둔다. 유닛을 누르면 남은 낱말이 펼쳐진다
  - 한두 장만 남은 유닛은 `바로 끝낼 수 있는 유닛`으로 위에 모아 둔다
- 현황판 입구를 떠 있는 버튼에서 독바 탭으로 옮김
- 독바를 웹에서 `position:fixed` 로 바꿔 늘 화면 아래에 붙게 함
- 독바에서 아이콘을 빼고 제목만 남김
- 앱 안에 그림 현황판 추가 (임시). 홈 우측 하단 떠 있는 버튼으로 들어간다
  - 코스 → 유닛 → 낱말 순으로 좁혀 보며, 아직 그리지 않은 낱말을 확인한다
  - 모든 학습 언어의 코스를 한 화면에서 보고, 그림이 등록되면 숫자가 곧바로 줄어든다
- 그림 생성기가 받아올 때 작업 중이던 수정을 잠시 치워 두도록 수정 (`--autostash`)
  - 커밋하지 않은 파일이 있으면 `git pull --rebase` 가 거부되어 올리기가 계속 밀렸음
- 토플 코스 그림 생성 시작 (931장 대상)
- 그림 생성 스크립트(`generate_images.py`)가 글자별 등록 파일 충돌만 났을 때 등록 파일을 다시 만들어 이어 가도록 수정
  - 여러 컴퓨터가 같은 글자를 그리면 `wordImagesByLetter/<글자>.ts` 끼리 자주 부딪혀 생성이 멈췄음. 그 밖의 파일 충돌은 그대로 멈춤
- 빈칸이 들어간 그림 파일 이름(`consist of.png`)을 붙임표로 바로잡음

### 2026-09-14
- 홈 코스 카드·유닛 카드의 그림 미완료 개수(임시 표시)가 그림을 새로 등록해도 옛 숫자로 남던 문제 수정: 등록된 그림 수가 바뀌면 홈은 개수를 다시 세고 유닛 목록은 셀을 다시 그림
...
- 토플 그림 없는 단어 180개의 영문 장면을 동작 중심으로 작성 (`scripts/tf_missing_scenes.json`), 카탈로그 갱신
  - 다른 작업의 커밋이 장면 반영 전 카탈로그로 덮어써 장면이 빠졌던 것을 다시 만들어 복구
...
- 중2 미생성 단어 1장 Draw Things 선형그래픽으로 생성 및 등록 (머리·몸통 분리 개정 스킬 적용)
  - 대상 단어: decide
- 고1 미생성 단어 5장 Draw Things 선형그래픽으로 생성 및 등록 (머리·몸통 분리 개정 스킬 적용)
  - 대상 단어: candidate, encounter, conference, policy, spin
