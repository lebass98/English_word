#!/usr/bin/env python3
"""
토익(TOEIC) 전용 선형그래픽 무중단 연속 생성 및 자동 배포 파이프라인.
...
사용법:
    python3 scripts/generate_toeic_continuous_linear.py --start-unit 3 --end-unit 106
"""
...
def load_data():
    ...
    return toeic_words, tr_ko, words_dict

def main():
    parser = argparse.ArgumentParser(description="토익 선형그래픽 무중단 연속 생성기")
    parser.add_argument("--start-unit", type=int, default=3, help="시작 유닛 (기본 3)")
    parser.add_argument("--end-unit", type=int, default=106, help="종료 유닛 (기본 75)")
    args = parser.parse_args()

    toeic_words, tr_ko, words_dict = load_data()
    total_words = len(toeic_words)
    total_units = (total_words + 19) // 20

    print("==================================================", flush=True)
    print(f"🚀 [토익 무중단 연속 생성 파이프라인 가동: Unit {args.start_unit} ~ Unit {args.end_unit}]", flush=True)
    ...
    for u in range(args.start_unit, args.end_unit + 1):
        ...
        unit_words = toeic_words[start_idx:end_idx]
        unit_tasks = []
        ...
        print(f"\n▶▶▶ [토익 Unit {u} / {total_units}] 20단어 파이프라인 가동 ◀◀◀", flush=True)
        ...
        for task in unit_tasks:
            ...
            try:
                ...
                commit_msg = f"feat: 토익 {u}단원 {w} ({meaning}) 선형그래픽 이미지 생성 및 등록"
        ...
        print(f"\n🎉 [토익 Unit {u}] 20단어 배포 전원 완료! 즉시 다음 유닛으로 진행합니다...\n", flush=True)

    print("\n🎊 토익 전체 유닛 연속 생성 작업이 완수되었습니다!", flush=True)
