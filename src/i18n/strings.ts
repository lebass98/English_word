 ...
 *
 * 여기 있는 건 "앱이 말하는 말"이다. 단어 뜻·예문 해석처럼 학습 콘텐츠에
 * 속하는 번역은 src/data/<학습언어>/tr/<표시언어>.json 에 따로 둔다.
 *
 * 새 표시 언어를 추가하려면 아래 STRINGS 에 항목을 하나 더 만들면 된다.
 * 빠진 문구는 자동으로 한국어로 대체되므로 한 번에 다 채우지 않아도 된다.
 */
...
/** 언어 고르는 화면에 쓸 이름. 각 언어는 제 나라 말로 적는다 */
...
const ko = {
  ...
  "settings.studyLangDesc": "지금 배우고 있는 말이에요",
...
const ja: Partial<Record<StringKey, string>> = {
  ...
  "settings.studyLangDesc": "いま学んでいる言語です",
